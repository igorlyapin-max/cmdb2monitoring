...
- `Monitoring Rules -> Profiles` for creating, editing, and deleting `hostProfiles[]`, followed by Conversion Rules Management for profile-bound rule conditions and Zabbix targets;
...
For `monitoring-ui-api`, external CMDBuild, Zabbix, IdP SAML metadata/OAuth2, and enabled PAM/AAPM endpoints use only `https://` in production. `MONITORING_UI_OUTBOUND_ALLOWED_HOSTS` is the exact hostname allowlist for those destinations; `http://`, URL-embedded credentials, an unlisted hostname, and disabled TLS verification are rejected. Development may use local HTTP topology.
...
`monitoring-ui-api` stores only the session id in the browser cookie with `SameSite=Strict`; HTTPS/Production also adds `Secure`. For HA, enable `Auth:SessionStore:Mode=Redis`; sensitive session payload is encrypted with `Auth:SessionEncryptionKey`/`MONITORING_UI_SESSION_ENCRYPTION_KEY`. In production Redis uses `MONITORING_UI_REDIS_URL=rediss://...` with TLS certificate validation. External CMDBuild, Zabbix, IdP SAML metadata/OAuth2, and enabled PAM destinations belong to deployment configuration: `MONITORING_UI_OUTBOUND_ALLOWED_HOSTS` is the exact allowed-host list, and session credentials cannot override endpoints.

## Production Docker Compose

The production Compose contract is in `deploy/compose.production.yml`; sample variables are in `deploy/production.env.example`. The target topology expects TLS termination or a reverse proxy in front of the containers: services listen on HTTP inside the Docker network with explicit `Transport__AllowPlainHttp=true`. For the BFF, external CMDBuild/Zabbix/IdP/PAM endpoints must use HTTPS and be listed in `MONITORING_UI_OUTBOUND_ALLOWED_HOSTS`; Kafka uses TLS/SASL unless an explicit development override is set.
...
- structured JSON logs to stdout/stderr and Kafka log topics; the platform selects the Docker logging driver, with syslog available through a separate overlay;
- required production env/secret inputs for webhook token, rules reload token, Kafka, CMDBuild, Zabbix, and audit storage.

When CMDBuild or Zabbix credentials are requested, the BFF displays the deployment-configured endpoint as reference. The user enters only login/password; the endpoint is neither sent by the browser nor allowed to change deployment topology.
...
| --- | --- | --- |
| CMDBuild | `4.2.x` | Target customer stand and product contract; REST API v4 and webhook JSON are used |
...
- CMDBuild webhook body remains flat JSON, and catalog/reference/lookup/domain data is readable through REST v4;
...
Indeed PAM/AAPM is optional: for deployment without PAM, use Secrets:Provider=None, empty PAM*/SASLPASSWORDSECRET, actual values from a protected deployment-layer source, and no secret:// or aapm:// references.
...
`CMDBuild Catalog` includes an editor/admin bulk lookup-change utility: the operator selects a class and writable lookup attribute, confirms the action, and the BFF reads class cards and sends per-card `PUT /classes/{class}/cards/{id}` calls. The `Only where zabbix_main_hostid is empty` filter is enabled by default, so cards already placed on monitoring are not changed; when this filter is enabled, the backend requires the selected class to expose this attribute. Cards are de-duplicated by id before writes; the result shows CMDBuild `meta.total`, the number of unique processed cards, and the number of cards filtered out by `zabbix_main_hostid`. The current lookup value is replaced with the next active lookup value, and empty values with the first active value. This is intended to explicitly generate `card_update_after` events for class cards, for example when a CMDBuild bulk update does not pass through webhooks; the CMDBuild user must have permission to update the selected attribute.
...
`zabbixrequests2api` protects resources owned by the suppression aggregate contour. With `Processing:ProtectManagedAggregateHosts=true`, the service does not send `host.create/update/delete` if the payload attempts to create/assign a protected marker, or if the host found before update/delete already has `cmdb2monitoring:aggregate=true` or a name from `Processing:ProtectedHostNames` (`cmdb2monitoring-suppression-aggregates` by default). This does not prevent updates to normal CMDB source hosts, even when suppression uses their triggers to recalculate aggregate dependencies.
...
| `Cmdbuild:ApiVersion` | Required value `v4`; legacy REST v3 is not supported |
...
- CMDBuild for `Webhook Setup` load/analyze: read access to ETL/webhook records through REST v4 `/administration/etl/webhook/?detailed=true`.
- CMDBuild for `Load into CMDB`: create/update/delete, or equivalent modify permissions, on ETL/webhook records through REST v4 `/administration/etl/webhook/`. These permissions are needed only by operators who actually apply webhook plans to CMDBuild; they are not needed by viewers or for ordinary catalog sync.
...
- CMDBuild for `Apply CMDBuild preparation` in the `Audit` section: CMDBuild model-administration permissions to create classes and attributes through `POST /administration/classes` and `POST /administration/classes/{class}/attributes`. These permissions are not needed for the audit plan check.
...
Rules format `schemaVersion: "0.2"` uses one mandatory `when.expression` tree. Group operators are `all`, `any`, and `not`; leaf operators are `always`, `equals`, `notEquals`, `regex`, `notRegex`, `exists`, and `empty`. `not` has exactly one child, and `all`/`any` have one or more children. Equality and regex operators never match blank input; use `empty` explicitly. The editor provides this tree as `AND`/`OR`/`NOT` groups and `Field | Operator | Value` rows. Legacy `allRegex`, `anyRegex`, `fieldExists`, `fieldsExist`, `fallback`, and `defaults.templates` are rejected by load, validation, and publish; customer rules must be converted before upgrading, with no automatic compatibility layer. Monitoring is explicit: no matching `hostProfiles[]` produces `no_host_profile_matched`; a matching profile with no matching `templateSelectionRules[]` produces `no_template_rule_matched` for `create` and `update`. Neither case emits a Zabbix request. A `delete` keeps running without templates so that it can clean up a previously created host. Route `fallbackMethod` remains only for resolving the Zabbix host id after a profile has already been selected.
...
Conversion Rules Management supports reference expansion, domain expansion, add/modify/delete rules, undo/redo, and `Save file as`. In edit mode the lower three-column preview is hidden; the CMDBuild class selector shows the class hierarchy with indentation and disables superclass/prototype classes, falling back to the nearest concrete subclass when a superclass was previously selected. Modification starts with no rule selected automatically; the operator can start from a rule, CMDBuild class, class attribute field, or conversion structure. Linked lists are filtered, and when a single matching rule remains it is selected automatically and loaded into the same editor form. The editor filters dependent fields, clears the leaf field and target after class changes, and saves changed class/field/structure/target/priority/regex/name back into draft JSON. Raw CMDBuild reference ids are not offered as IP/DNS leaves; operators choose the resolved nested leaf instead. When two classes have same-named leaves, the editor keeps separate internal source keys but shows configured `cmdbPath` fields in dropdowns as readable paths such as `mgmt -> ipAddr / routeCore`; generated keys like `routeCoreMgmtIpAddr` remain in JSON/tooltips. CMDBuild 1:N domains that are already exposed as reference attributes on the selected class are hidden from leaf choices, so the same related attributes are not offered through both reference and domain paths; real N:N relations remain available. The add/modify editor exposes virtual `hostProfile` and `outputProfile` fields; the converter fills them for each `hostProfiles[]` entry, so they can restrict a template/group/tag rule to a specific fan-out profile. `outputProfile` is not valid in a profile-scoped rule. When `Limit rule to selected hostProfile` is enabled, the `CMDBuild class` selector is hidden, a `hostProfile / class` context is shown instead, the field selector is labelled with that class, `className` is not added to the condition, and the condition builder offers only fields compatible with that profile class plus `eventType` and `hostProfile`; a global rule keeps independent class and field selection. A scoped rule must contain one positive `hostProfile = ...` condition in the mandatory root `all` branch and cannot read a field whose configured `cmdbPath` belongs to another CMDBuild class. For `interfaceAddress`, the editor validates target semantics: an IP-looking CMDBuild attribute cannot be saved as DNS target `interfaces[].dns/useip=0`, a DNS/FQDN-looking attribute cannot be saved as IP target `interfaces[].ip/useip=1`, and an unconfirmed address field must be made explicit through name/source metadata or `validationRegex`; for `cmdbPath`, address semantics are detected from the final leaf, not from the reference-branch name. For `Tag rule` and `Host group rule`, the editor can save a dynamic target from a CMDBuild leaf only when the matching runtime switch is enabled; in the UI this is a separate target option, not an empty field. For templates, interfaces, inventory, and macros, an empty target remains an error. Template targets are additionally checked against `Zabbix Metadata`: if the selected template plus defaults/selected templates after `templateConflictRules` still leaves a duplicate item key, duplicate LLD rule key, or duplicate inventory link, the target is marked red and save is blocked. `Reset fields` clears the selected rule and filters in modify mode and clears leaf/target in add mode. Green borders mean compatible, red borders mean required/conflicting, and yellow borders mean the value came from the rule but is not confirmed by the current catalog/filter. `Current rule target / missing from Zabbix catalog` is inconsistent, shown in red, and blocks saving just like a missing CMDBuild class/attribute. The dedicated `Monitoring profiles` block manages `hostProfiles[]`: the operator selects CMDBuild class, `Main`/`Additional` profile type, IP/DNS leaf, IP/DNS mode, local Zabbix `interfaces[]` profile, and `createOnUpdateWhenMissing`. Adding or modifying a normal conversion rule no longer creates `hostProfiles[]` implicitly; if a class has no applicable profile, Logical Control reports the `no_host_profile_matched` risk and the profile must be created in the dedicated block. An additional profile is created as a separate fan-out profile with the `HostProfileName` suffix, a filled-leaf-or-`delete` condition, its own `interfaces[].valueField`, and the selected `interfaceProfileRef`; when an existing profile is renamed, exact `hostProfile` conditions in scoped rules are renamed too. Deleting a profile from this block removes `hostProfiles[]` and rules explicitly scoped to that `hostProfile`; it does not delete a Zabbix host in the managed system. The `Assignments` counter counts only rules with an explicit `hostProfile` condition. Logical Control highlights missing CMDBuild/Zabbix elements, also treats a source class without a matching `hostProfiles[]` as a rules error that would become `no_host_profile_matched`, highlights incompatible Zabbix template sets found through `Zabbix Metadata`, and, when CMDBuild webhooks are loaded in the current UI session, shows warnings for missing managed webhooks or payload fields required by rules; the operator saves the corrected JSON through the browser.

### Profile-bound selection rules

The current rule editor has no global selection-rule mode; this contract supersedes the older generic editor description above. Its form always shows the required monitoring-profile list and the object-condition tree. Until the operator selects one existing `hostProfile`, the fixed constraint reads `hostProfile = not selected`, the tree is disabled, and saving is blocked. Once selected, the CMDBuild class is derived from that profile and is not an editable field. The selected profile is displayed separately as the fixed `hostProfile = ...` constraint and is written into the mandatory root `all` branch when the rule is saved. Operators edit a separate object-condition tree using `all`, `any`, `not`, `equals`, `notEquals`, `regex`, `notRegex`, `exists`, and `empty`; the regex operators take a regular expression as their value. Only fields compatible with the profile class and `eventType` may be used in that tree. The CMDBuild field selector is shown only for a `dynamicFromLeaf` target and is labelled as its dynamic value field. Existing selection rules that do not satisfy this contract, or reference a missing profile, remain ignored by the converter and block BFF validation/publish until corrected. No automatic migration or fallback is provided.

If an existing `hostProfiles[]` entry is present but its `interfaces[].valueField` still points to a raw reference id or another invalid address field, Logical Control can replace that field with a class-scoped IP/DNS leaf in the in-session draft.

`Webhook Setup` is available to `editor` and `admin`. Using it is optional: operators can configure webhooks manually in CMDBuild or use the webhook files saved together with the conversion rules file. `Load from CMDB` reads current CMDBuild ETL webhooks through the BFF and session-scoped CMDBuild credentials; after load, current webhooks are shown for reference, and the create/update/delete plan is built separately by `Analyze rules`. `Analyze rules` reloads the current conversion rules and CMDBuild catalog cache every time, treats conversion rules as the source of truth for webhook payload, builds `webhook requirements` for all used source fields, and derives desired webhooks from those requirements. When rules use a lookup/reference/domain leaf, the webhook payload receives only the nearest required CMDBuild placeholder for the current card, while the converter later resolves the leaf by `source.fields[].cmdbPath` metadata. Missing lifecycle webhook records are proposed as `Create` with the namespaced code `cmdbwebhooks2kafka-zabbix-host-<class>-<event>`, records with different body/event/target/method/url/non-Authorization headers/active/language as `Update`, and obsolete owned `cmdbwebhooks2kafka-*` records, including old short-code lifecycle records, as `Delete`. If an existing CMDBuild webhook lacks payload fields required by rules for that class, the summary, row details, and operation reason show the concrete payload keys and the rules that require them; without applying that operation or manually updating the webhook, those values will not reach the Kafka event and will not be available to the converter. Each table row can expand its payload: green marks added values, red marks deleted values, and black marks current values. Clicking the value in the `Action` column opens details under that same row; the shared `Details` panel is below the table and uses the same highlighting for current/desired/delete text. When existing webhooks are analyzed, their loaded body is preserved and optional source fields are added only when rules for that class actually need them; adding an independent class must not turn unrelated hooks into `Update`. The UI must not add a duplicate key with another case or alias, for example `OS` next to an existing `os`. A `cmdbPath` rooted in another class, for example `OtherClass.Attribute1.Attribute2`, must not create a placeholder in the current class webhook. The row `Edit` button opens JSON for that concrete webhook; saving the edit changes only the current plan, and for a loaded CMDB row it creates a selected `update` operation. Delete operations are not selected by default. `Delete selected` applies only selected `Delete` operations and does not send `Create`/`Update` operations; other changes are applied by the general `Load into CMDB` button. Undo/redo works on the in-session operation selection and does not roll back an already executed `Load into CMDB` command, because that command changes the managed system. `Save file as` exports only the JSON plan through the browser and does not change CMDBuild, the backend rules file, or git; token/password/secret/API key/Authorization values in exported webhook JSON are masked as `XXXXX`. `Load into CMDB` applies only selected operations through REST v4 `/administration/etl/webhook/`, but it preserves existing `headers.Authorization` during ordinary updates. Authorization rotation is a separate `Sync Authorization` operation that updates only that header on owned managed webhooks, using `Cmdbuild:Webhooks:AuthorizationHeader` or `CMDBUILD_WEBHOOK_AUTHORIZATION_HEADER`. The backend restricts this workflow to the managed `cmdbwebhooks2kafka-` prefix, and for `update`/`delete` it reloads current CMDBuild webhooks and applies the action only to the found owned managed record with the same `code`; webhooks owned by other microservices are not overwritten or deleted.

One `Load into CMDB` request accepts no more than 100 operations. Above the limit, the BFF rejects the complete batch before contacting CMDBuild.
...
To add a new fixed IP to the main host, add a CMDBuild attribute, webhook field, `source.fields`, and a `hostProfiles[].interfaces` entry. To add a separate monitoring profile, add a named source field and a separate `hostProfile` with its own suffix; create it through the first `Monitoring Rules -> Profiles` menu item, then choose it in the Conversion Rules Management form before assigning templates/groups/tags with profile-bound object conditions. Arbitrary or unknown-size IP arrays in a single webhook field are not currently supported.
...
By default it targets the dev environment: `C2MTestCI`, webhook `http://localhost:5080/webhooks/cmdbuild`, and Zabbix `http://localhost:8081/api_jsonrpc.php`. Credentials have no built-in values; provide an API token or login/password through CLI arguments or `C2M_SMOKE_*` environment variables. Production-like execution requires explicit `--environment prod`, a test `--code` with the `C2M-SMOKE-` prefix, and matching `--confirm`. The script checks `/ready`, reloads rules, cleans up an old test host, sends `create`, `update`, and `delete` webhooks, and verifies the final state through the Zabbix API.

## Extended Debug Logging

All backend services, including Node.js `monitoring-ui-api`, support the `DebugLogging` section with `Basic` and `Verbose` levels. Extended debug events are emitted through the main structured logging pipeline at `Information`, so they reach Docker stdout/stderr, Kafka log topics while `ElkLogging:Mode=Kafka`, ELK when the ELK sink is enabled, and syslog if Docker uses a syslog logging driver. `Verbose` emits only safe payload metadata: byte size, SHA-256 fingerprint, and field names. It never emits raw CMDB/Zabbix values, `Authorization`, tokens, passwords, secrets, API keys, private keys, or connection strings.
...
`rules/cmdbuild-to-zabbix-host-create.json` remains the populated demo/e2e rules file for dev verification only. It is populated only for the `C2MTestCI` test model and related reference/domain leaf paths. Production Compose forces `CONVERSION_RULES_FILE_PATH=rules/cmdbuild-to-zabbix-host-create.production-empty.json` for both converter and UI; a customer deployment may replace it only with a prepared customer rules file. `rules/cmdbuild-to-zabbix-host-create.dev-empty.json` is the clean dev no-op starter generated from the empty installation profile; it already points to `cmdbuild.webhooks.dev` and `http://localhost:8081/api_jsonrpc.php`. Both starter files keep routes at `publish=false` until the operator fills classes, source fields, host profiles, Zabbix catalog IDs, T4 templates, and enables publication after verification. To work with the empty dev starter, load it in the UI or point `Rules.RulesFilePath` to it through config/env or Runtime settings.

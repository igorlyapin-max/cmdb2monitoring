...
- добавление/модификация обычного conversion rule больше не создает `hostProfiles[]` скрыто; если class не имеет применимого profile, logical control покажет риск `no_host_profile_matched`, а profile нужно создать в отдельном блоке; при добавлении rule выбранный profile автоматически подставляет свой CMDBuild class в форму rule;

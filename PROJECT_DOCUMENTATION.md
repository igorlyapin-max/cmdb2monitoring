...
- приоритет 2: сохраненная обратная связь в CMDBuild. Для основного профиля `cmdbkafka2zabbix` читает исходную карточку и берет `Cmdbuild:MainHostIdAttributeName`, default `zabbix_main_hostid`. Для дополнительного профиля читает `Cmdbuild:BindingClassName`, default `ZabbixHostBinding`, и точечно ищет активную карточку по `OwnerClass + OwnerCardId + HostProfile`, без полного чтения binding-класса;
...
В `Runtime-настройки` также есть две независимые галки для редактора правил: `Разрешить динамическое расширение Zabbix Tags из CMDBuild leaf` и `Разрешить динамическое создание Zabbix Host groups из CMDBuild leaf`. При выключенной галке редактор требует выбрать существующий Zabbix target. При включенной галке для соответствующей conversion structure появляется явный target `Создавать/расширять из выбранного CMDBuild leaf`; сохраненный rule содержит `targetMode=dynamicFromLeaf`, `valueField` и `createIfMissing`. В форме add/modify для таких targets, а также для scalar payload objects вроде tag value, host macro value и inventory field value, доступен блок `Строка payload`: пустая строка означает прямое значение выбранного leaf, а непустая сохраняется как `nameTemplate` или `valueTemplate`. Шаблон может собираться из любых доступных leaf-полей текущего класса через `Model.Source("field")` и `Model.Regex("field", "pattern")`/`Model.Regex("field", "pattern", "replacement")`; поля, пришедшие через reference/domain/lookup, сохраняют обычный `source.fields[].cmdbPath` и разрешаются converter перед рендерингом шаблона. Этот режим намеренно не распространяется на templates и interfaces. Для host groups это означает не только создание/поиск справочника: при первом появлении leaf-значения writer создает отсутствующую group, подставляет полученный `groupid` в тот же `host.create`/`host.update` payload и тем самым сразу привязывает текущий host к этой group. Динамическое расширение нужно включать только после анализа разнообразия leaf-значений: неконтролируемые изменения атрибутов CMDBuild, по которым выполняется mapping, дадут такой же объем динамических изменений в Zabbix.
...
Runtime cache карточек CMDBuild и domain/reference relations действует только внутри одного события resolver. Поэтому при модификации исходной карточки и поступлении `card_update_after` converter заново читает leaf-значения по reference и domain путям. CMDBuild lookup values дополнительно имеют сервисный TTL-кэш `Cmdbuild:LookupCacheTtlSeconds` (default `300`, `0` отключает), потому что справочники меняются редко, а повторное чтение lookup type на каждом объекте заметно увеличивает задержку. Это позволяет обновить Zabbix по изменившимся связанным значениям, если событие update пришло именно по исходной карточке; изменение самого справочника CMDBuild может потребовать дождаться TTL или перезапустить сервис. Одна только модификация связанной карточки или domain-связи без webhook/event исходной карточки не запускает пересчет мониторинга.

В разделе `CMDBuild Catalog` для ролей `editor` и `admin` есть операторская утилита массового изменения lookup: оператор выбирает класс и writable lookup-атрибут, подтверждает действие, после чего BFF читает карточки класса и выполняет поштучные `PUT /classes/{class}/cards/{id}`. По умолчанию включен фильтр `Только где пустой zabbix_main_hostid`, поэтому уже поставленные на мониторинг карточки не изменяются; если фильтр включен, backend требует наличие этого атрибута в выбранном классе. Карточки перед записью дедуплицируются по id; результат показывает `meta.total` CMDBuild, число уникально обработанных карточек и количество карточек, отфильтрованных по `zabbix_main_hostid`. Текущее значение lookup заменяется следующим активным значением справочника, пустое значение - первым активным. Утилита предназначена для явной генерации `card_update_after` по карточкам класса, например когда CMDBuild bulk update не отработал через webhooks; CMDBuild-пользователь должен иметь право изменять выбранный атрибут.
...
Проверки Zabbix host groups и templates используют positive TTL-кэш: найденные host group ids/names и template metadata переиспользуются между объектами до истечения `Zabbix:HostGroupCacheTtlSeconds` и `Zabbix:TemplateCacheTtlSeconds`. Отсутствующие значения не кэшируются, чтобы newly created group/template мог появиться без ожидания TTL. При `hostgroup.create` новый groupid сразу попадает в кэш.

`zabbixrequests2api` защищает ресурсы, которыми владеет suppression aggregate contour. При `Processing:ProtectManagedAggregateHosts=true` сервис не отправляет `host.create/update/delete`, если payload пытается создать/назначить защищенный marker или найденный перед update/delete host уже имеет `cmdb2monitoring:aggregate=true` либо имя из `Processing:ProtectedHostNames` (`cmdb2monitoring-suppression-aggregates` по умолчанию). Это не запрещает менять обычные CMDB source hosts, даже если suppression использует их triggers для пересчета aggregate dependencies.
...
- перед записью сервис сравнивает текущее CMDBuild-состояние с binding event и пропускает `PUT`, если `zabbix_main_hostid` или найденная карточка `ZabbixHostBinding` уже совпадает. Это снижает шум от повторных binding events и не создает лишние CMDBuild update webhooks;
...
- `schemaVersion: "0.2"` использует единый обязательный формат условия `when.expression`. Операторы групп: `all`, `any`, `not`; листовые операторы: `always`, `equals`, `notEquals`, `regex`, `notRegex`, `exists`, `empty`. `not` содержит ровно один элемент, `all`/`any` - один или больше. `equals`/`notEquals` и regex-варианты не срабатывают на пустом значении; для пустоты используется только `empty`.
- Пример вложенной логики: `{ "operator": "any", "items": [{ "operator": "all", "items": [{ "operator": "equals", "field": "criticality", "value": "High" }, { "operator": "notEquals", "field": "lifecycle", "value": "Retired" }] }, { "operator": "regex", "field": "service", "pattern": "(?i)^network$" }] }` означает `(criticality = High И lifecycle != Retired) ИЛИ service regex ^network$`.
- Старые `allRegex`, `anyRegex`, `fieldExists`, `fieldsExist`, `fallback` и `defaults.templates` не поддерживаются: загрузка, проверка и публикация такого файла завершаются ошибкой. Старый customer rules-файл нужно преобразовать до обновления, автоматической совместимости нет.
- Мониторинг выбирается только явными правилами: нет совпавшего `hostProfiles[]` - `no_host_profile_matched`; есть profile, но для `create`/`update` ни один `templateSelectionRules[]` не выбрал template - `no_template_rule_matched`; в обоих случаях запрос в Zabbix не создается. Для `delete` отсутствие template не блокирует cleanup ранее созданного host. `fallbackMethod` маршрута остаётся отдельным механизмом восстановления `hostid` для уже выбранного profile.
...
- центральная колонка показывает conversion fields, дерево условий, selection rules и T4 blocks;
- Host profiles в центральной колонке показывают fan-out и конкретные связи `профиль Zabbix interfaces[]`/`valueField`;
- редактор правил добавления/модификации всегда показывает обязательный список `Профиль мониторинга` и блок условий; до выбора профиля системная строка показывает `hostProfile = не выбран`, дерево условий неактивно, а сохранение заблокировано. После выбора profile класс определяется автоматически, а системное условие `hostProfile = ...` создаётся при сохранении в корневой обязательной ветке `И`; `outputProfile` не используется для назначения rules;
...
- добавление и модификация правила используют визуальный builder пользовательского дерева условий: группы `И`/`ИЛИ`/`НЕ`, строки `Поле | Оператор | Значение`, где доступны `=`, `!=`, `regex`, `not regex`, `заполнено`, `пусто`; `regex` и `not regex` принимают регулярное выражение в значении. Список profile расположен в самой форме правила, а системное условие `hostProfile = ...` отображается отдельно и без возможности редактирования; при сохранении оно добавляется в корневую ветку `И` вместе с пользовательским деревом. `Класс CMDBuild` в форме не показывается и однозначно выводится из выбранного `hostProfile`, поэтому доступны только поля этого класса и `eventType`; поле выбора CMDBuild отображается только для `dynamicFromLeaf` target как `Поле для динамического значения`; global rule и выбор класса вне profile отсутствуют;
...
- `Save file as` сохраняет draft JSON с новым `rulesVersion` и второй текстовый файл с CMDBuild webhook Body/DELETE-инструкциями только по добавленным и удаленным в текущей сессии rules/classes/source fields;
...
- перед сохранением проверяется, что каждый мониторинговый класс из `source.entityClasses` или условия `className` имеет IP или DNS class attribute field, связанный с `interfaceAddressRules` или `hostProfiles[].interfaces`, и применимый `hostProfiles[]`; для фактической постановки на мониторинг также нужен явный `templateSelectionRules[]`, иначе converter пропустит событие с `no_host_profile_matched` или `no_template_rule_matched`;
...
- добавление/модификация обычного conversion rule больше не создаёт `hostProfiles[]` скрыто и запрещена без выбранного существующего profile; если class не имеет применимого profile, logical control покажет риск `no_host_profile_matched`, а profile нужно создать в отдельном блоке; выбранный profile фиксирует CMDBuild class и отсекает несвязанные class attribute fields;
...
- удаление profile в меню `Правила мониторинга -> Профили` удаляет `hostProfiles[]` и rules, явно ограниченные этим `hostProfile`; это не удаляет Zabbix host в управляемой системе;
- templates, groups, tags и другие selection rules назначаются только выбранному `hostProfile`: например, после создания profile `класс-профиль` в меню `Правила мониторинга -> Профили` выберите его в форме `Управление правилами конвертации` и добавьте `Правило template` с нужными условиями по полям этого класса (`description`, lookup, domain leaf и т.д.); такой rule обязан содержать ровно одно положительное условие `hostProfile = ...` в корневой ветке `И`, не может содержать `className` и не может читать поле другого CMDBuild class; счетчик `Назначения` считает только такие rules. Существующие selection rules без этого условия или со ссылкой на удалённый profile не мигрируются и не получают fallback: converter игнорирует их, а BFF блокирует validate/publish до исправления.
...
- новые lifecycle webhook records всегда предлагаются с namespaced code `cmdbwebhooks2kafka-zabbix-host-<class>-<event>` как `Создать`, существующие свои records с отличающимся body/event/target/method/url/неавторизационными headers/active/language - как `Изменить`, свои старые short-code records `cmdbwebhooks2kafka-<class>-<event>` и любые другие owned `cmdbwebhooks2kafka-*`, которые больше не нужны по rules, - как `Удалить`; отличие только `headers.Authorization` в обычный create/update/delete-план не входит;
...
RULES_FILE_PATH=rules/cmdbuild-to-zabbix-host-create.production-empty.json
...
- чтобы добавить еще один отдельный monitoring profile, нужно добавить новый named source field и отдельный `hostProfile` с собственным suffix, например `separate-profile-3`; через первое меню `Правила мониторинга -> Профили` создается профиль, затем он выбирается в форме правила, а templates/groups/tags назначаются отдельными profile-bound rules с условиями объекта; при необходимости upsert на update включается `createOnUpdateWhenMissing=true`;
...
`Basic` включает диагностическую трассу прохождения события между webhook, Kafka, converter, Zabbix API, обратной записью binding в CMDBuild и startup/runtime состоянием BFF. `Verbose` дополнительно пишет только безопасные metadata payload: размер, SHA-256 fingerprint и имена полей; исходные CMDB/Zabbix значения, `Authorization`, tokens, passwords, secrets, API keys, private keys и connection strings в него не попадают. Включать его нужно только на время диагностики.
...
Для сравнения производительности стендов есть read-only baseline:

```bash
node scripts/perf-baseline.mjs --samples 25
```

Скрипт пишет `reports/perf-baseline-*.json` и измеряет типовые REST/JSON-RPC операции CMDBuild/Zabbix без изменения внешних систем. Baseline имеет смысл хранить как артефакт проверки конкретного стенда; при замене стенда старые числа пригодны только как ориентир, а не как строгий regression gate.
...
Fast regression-набор `tests/cmdbresolver` входит в `./scripts/test-configs.sh` и не требует живых CMDBuild/Zabbix/Kafka. Он проверяет, что два последовательных update события с одним экземпляром resolver заново читают reference leaf cards и domain leaf cards, а lookup values перечитываются при `Cmdbuild:LookupCacheTtlSeconds=0` и переиспользуются при включенном TTL. Отдельный сценарий проверяет весь путь до converter output: обновленное domain leaf значение должно попасть в dynamic `groups[]` как host group с `createIfMissing=true`.
...
Файл `rules/cmdbuild-to-zabbix-host-create.json` остается demo/e2e-набором правил только для dev-проверок. Базовый dev-набор построен вокруг абстрактной тестовой модели `C2MTest*`, но в ходе проверки в него можно добавлять любые конкретные классы текущего CMDBuild catalog: это не ограничение продукта и не признак жесткой привязки к именам модели. Для каждого такого класса должны быть заполнены `source.entityClasses`, source field для IP/DNS leaf, применимый `hostProfiles[]` и webhooks. Production Compose принудительно задает `CONVERSION_RULES_FILE_PATH=rules/cmdbuild-to-zabbix-host-create.production-empty.json` для converter и UI; customer deployment должен заменить его только подготовленным customer rules-файлом. Чистый dev starter лежит рядом: `rules/cmdbuild-to-zabbix-host-create.dev-empty.json`; он создан из пустого installation profile, но уже указывает `cmdbuild.webhooks.dev` и `http://localhost:8081/api_jsonrpc.php`. Оба starter-файла безопасны для запуска как no-op: routes имеют `publish=false`, а оператор должен заполнить классы, source fields, host profiles, Zabbix catalog IDs, T4 templates и включить публикацию только после проверки. Чтобы работать именно с пустым dev starter, его нужно загрузить в UI или указать в `Rules.RulesFilePath` через config/env или `Runtime-настройки`.

...
| SEC-021 | IF-014, IF-018, IF-021, IF-022 | Indeed PAM/AAPM bootstrap credentials | `Secrets:IndeedPamAapm:ApplicationToken`, `Secrets:IndeedPamAapm:ApplicationTokenFile`, `Secrets:IndeedPamAapm:ApplicationUsername`, `Secrets:IndeedPamAapm:ApplicationPassword`, aliases `PAMURL`, `PAMUSERNAME`, `PAMPASSWORD`, `PAMTOKEN` | По политике PAM/AAPM | Нужны только для получения сервисных секретов по `secret://id`; хранить в Docker/Kubernetes secret, защищенном mount или env secret, не в git |
| SEC-022 | IF-002, IF-003, IF-006, IF-015, IF-021 | Kafka SASL corporate aliases | `SASLUSERNAME`, `SASLPASSWORD`, `SASLPASSWORDSECRET` | По политике Kafka/PAM | Заполняют пустые Kafka SASL поля; `SASLPASSWORDSECRET` преобразуется в `secret://id`, например `AAA.LOCAL\PROD.contractorProfiles` |

Production/base config не должен хранить реальные секреты. Использовать переменные окружения, secret storage, Indeed PAM/AAPM references `secret://id` или local config, исключенный из git.

...
- `monitoring-ui-api` хранит `Services:HealthEndpoints[].RulesReloadUrl` и `RulesReloadToken`, вызывает endpoint из dashboard-карточки `cmdbkafka2zabbix`.
...
| `Rules:ReadFromGit` | Галка `Читать из git` в Runtime-настройках UI | Для отображения выбранного режима работы с rules в UI/BFF |
...
| `Services:HealthEndpoints` | Health endpoints микросервисов; для `cmdbkafka2zabbix` дополнительно `RulesReloadUrl` и `RulesReloadToken` | При добавлении сервисов или reload-действий |
...
- Zabbix для UI/catalog sync: API access и read-only доступ к host groups, template groups, templates, hosts/tags и optional catalogs, читаемым через `*.get`.

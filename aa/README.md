...
Текущая версия описывает dev-контур и целевую структуру для тест/прод, включая `Управление правилами конвертации`, `Логический контроль правил конвертации`, сохранение языка интерфейса в cookie и lookup/reference path resolution через `cmdbPath`. Kafka topics создаются внешней инфраструктурой, микросервисы topics не создают.
